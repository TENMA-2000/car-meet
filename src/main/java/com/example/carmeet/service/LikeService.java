...
public class LikeService {
	...
	public Like addLike(Post post, User user) {
		...
		log.debug("いいね保存成功 likeId=" + savedLike.getLikeId() + ", postId=" + post.getPostId() + ", userId="
				+ user.getUserId());
	...
	public boolean checkUserLiked(Post post, User user) {
		...
		log.debug("checkUserLiked呼び出し postId=" + post.getPostId() + ", userId=" + user.getUserId() + ", 既にいいねしているか="
				+ exists);
		return exists;
	}
}
